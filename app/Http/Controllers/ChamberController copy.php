...
namespace App\Http\ChamberControllerBck;
...
class ChamberControllerBck extends Controller
{
    private function processChambersData124($unique_id, $startDate, $endDate)
    {
        ...
        foreach ($unique_id as $service_id) {
            ...
            while ($currentDate->lte($endDate)) {
                ...
                if ($entries->isNotEmpty()) {
                    ...
                    if (is_numeric($row['first_row_tel_temperature']) && $row['second_row_tel_temperature'] == 'NW') {
                    ...
                    if ($row['first_row_tel_temperature'] == 'NW' && is_numeric($row['second_row_tel_temperature'])) {
    ...
    private function processChambersData($unique_id, $startDate, $endDate, $isExport = false)
{
    $chambers = Chamber::whereIn('sys_service_id', $unique_id)
        ->whereBetween('date', [
            $startDate->format('Y-m-d'),
            ...
            if ($entries->isNotEmpty()) {
                ...
                if (is_numeric($row['first_row_tel_temperature']) && ($row['second_row_tel_temperature'] == 'NW' || $row['second_row_tel_temperature'] > 40)) {
                    ...
                    if ($maxTempEntry) {
                        ...
                        // Only execute the insert logic if it's not an export
                        if (!$isExport) {
                        ...
                        // Set second row tel_temperature to 'NW' if necessary
...
public function index()
{
    ...
    $endDate = Carbon::createFromFormat('Y-m-d', '2024-04-30');

    $formattedChambers = $this->processChambersData($unique_id, $startDate, $endDate, true); // Set isExport to false
...
public function export()
{
    ...
    $formattedChambers = $this->processChambersData($unique_id, $startDate, $endDate, true); // Set isExport to true

    return Excel::download(new ChambersExport($formattedChambers), 'chambers.xlsx');
}


    // private function processChambersData($unique_id, $startDate, $endDate)
    // {
    //     $chambers = Chamber::whereIn('sys_service_id', $unique_id)
    //         ->whereBetween('date', [
    //             $startDate->format('Y-m-d'),
    ...
    //                     if ($telemetryData) {
    //                         // Start time is 12:00:00, and we want to increment by 1 minute until 23:59:59
    //                         $startTime = Carbon::createFromFormat('H:i:s', '12:00:00');

    //                         // Loop through every minute from 12:00 to 23:59 (1440/2 = 720 iterations for afternoon)
    //                         for ($i = 0; $i < 720; $i++) {
    //                             $currentGpsTime = $startTime->copy()->addMinute($i);
    //                             DB::table('telemetry_new_chamber')->insert([
    ...
    //     // $unique_id = [135892,147165,63245,62423,54430,63874,63325,147247,135514,63043,62476,63278,63737,63881,63243,62555,55636,63809,63259,55116,52227,135102,52228,54200,52226,187168,147160,128200,128353,52209,52345,52342,52344,52343,63273,63274,63262,175275,174736,175034,173366,52348,63166,129359,54454,127943,63201,166035,63422,63518,63033,63032,63056,63329,70802,63199,63322,128066,63842,63841,63239,141430,141433,63756,64027,64031,64029,63203,54374,54786,63489,63162,63937,63936,63935,63938,63324,52282,151346,51078,52280,63159,128827,12422045,12422043,63037,63666,64032,64030,64028,63242,68183,63836,53995,63814,136480];
    //     // $unique_id = [63813];
    //     // $unique_id = [63330,63904,63812,63216,63905,63813,12422047,63870,12422046,63872,63736,63031,63035];
63710, 63452, 63509, 186916, 12435327, 63522, 63871, 63345, 63420, 141453, 63344, 12435325, 12435329, 64229, 63343, 55656, 12435324, 12435328, 63120, 63346, 63237, 54429, 55215, 128808, 63843, 141431, 63240, 63549, 141432, 141399, 63427, 64264, 63348, 63423, 63323, 129072, 63839, 56290, 135910, 63408, 129360, 63643, 63491, 62364, 63873, 63479, 63503, 63604, 63499, 63605, 63165, 63184, 63428, 63492, 63206, 63277, 63167, 63038, 63779, 63217, 63490, 63246, 63780, 63251, 55310, 63136, 62392, 63249, 63238, 63392, 63797, 63163, 63419, 135082, 63135, 63472, 63642, 63514, 63321, 63036, 63487, 55147, 63521, 63838, 63276, 63040, 63393, 64071, 70695, 63042, 63972, 58530, 63406, 63039, 63194, 129002, 56312, 63488, 135909, 63044, 63119, 63646, 63193, 63978, 63524, 63877, 63667, 63601, 62393, 63480, 63818, 63218, 63421, 63463, 63400, 63502, 54861, 63195, 63328, 62451, 63878, 63817, 53996, 63665, 63154, 63440, 54268, 63404, 63429, 63552, 63430, 63347, 63405, 63241, 63045, 64048, 63355, 63886, 147335, 63041, 63157, 63200, 135302, 63155, 63391, 63236, 63516, 63250, 70752, 63515, 63875, 54266, 147162, 70728, 63394, 63525, 147163, 63501, 54267, 63407, 147164, 63880, 63164, 54906, 63741, 147161, 135892, 147165, 63245, 62423, 54430, 63874, 63325, 147247, 135514, 63043, 62476, 63278, 63737, 63881, 63243, 62555, 55636, 63809, 63259, 55116, 52227, 135102, 52228, 54200, 52226, 187168, 147160, 128200, 128353, 52209, 52345, 52342, 52344, 52343, 63273, 63274, 63262, 175275, 174736, 175034, 173366, 52348, 63166, 129359, 54454, 127943, 63201, 166035, 63422, 63518, 63033, 63032, 63056, 63329, 70802, 63199, 63322, 128066, 63842, 63841, 63239, 141430, 141433, 63756, 64027, 64031, 64029, 63203, 54374, 54786, 63489, 63162, 63937, 63936, 63935, 63938, 63324, 52282, 151346, 51078, 52280, 63159, 128827, 12422045, 12422043, 63037, 63666, 64032, 64030, 64028, 63242, 68183, 63836, 53995, 63814, 136480];
    //     // $unique_id = SystemServiceUniqueId::select('unique_id')->pluck('unique_id')->toArray();
    ...
    // public function index()
    // {
    //     $unique_id = [63330,63904,63812,63216,63905,63813,12422047,63870,64028,63242,68183,63836,53995,63814,136480];
    ...
    //     // $tempSummary = TemperatureSummary::where('sys_service_id', $unique_id)
    //     // ->where('temp_date', $chambers->date);
    ...
    //                 'first_row_time' => 'N/A',
    //                 'first_row_gps_time' => 'N/A',
    //                 'first_row_tel_temperature' => 'N/A',
    //                 'second_row_date' => 'N/A',
    //                 'second_row_time' => 'N/A',
    //                 'second_row_gps_time' => 'N/A',
    //                 'second_row_tel_temperature' => 'N/A',
    //             ];
    ...
    //                         $row['first_row_time'] = $chamberEntry->time;
    //                         $row['first_row_gps_time'] = $chamberEntry->gps_time;
    //                         // $row['first_row_tel_temperature'] = $chamberEntry->tel_temperature;

    //                         $row['first_row_tel_temperature'] = ($chamberEntry->tel_temperature == 0) ? 'NW0M' : $chamberEntry->tel_temperature;

    //                     } elseif ($i == 1) {
    //                         $row['second_row_date'] = $chamberEntry->date;
    //                         $row['second_row_time'] = $chamberEntry->time;
    //                         $row['second_row_gps_time'] = $chamberEntry->gps_time;
    //                         $row['second_row_tel_temperature'] = ($chamberEntry->tel_temperature == 0) ? 'NW0A' : $chamberEntry->tel_temperature;
    //                     }
    //                     if($row['first_row_tel_temperature']== 'NW0M' && $row['second_row_tel_temperature']=='NW0A'){
    //                         $row['first_row_tel_temperature'] = 'NW0';
    //                         $row['second_row_tel_temperature'] = 'NW0';
    //                     }
    //                     // if(is_numeric($row['first_row_tel_temperature']) && $row['second_row_tel_temperature']=='N/A'){
    //                     //     $row['second_row_tel_temperature'] = 'Aaaa';
    //                     // }

    //                     if(is_numeric($row['first_row_tel_temperature']) && $row['second_row_tel_temperature'] == 'N/A') {
    //                         // Query the TemperatureSummary for the max_temp
    //                         $maxTempEntry = TemperatureSummary::where('sys_service_id', $service_id)
    //                             ->where('temp_date', $currentDate->format('Y-m-d'))
    //                             ->first();
    ...
    //                             $row['second_row_tel_temperature'] = $maxTempEntry->max_temp;
    //                         } else {
    //                             $row['second_row_tel_temperature'] = 'Not Found';
    //                         }
    //                     }
    //                     if($row['first_row_tel_temperature']=='N/A' && is_numeric($row['second_row_tel_temperature'])) {
    //                         // Query the TemperatureSummary for the min_temp
    //                         $maxTempEntry = TemperatureSummary::where('sys_service_id', $service_id)
    //                             ->where('temp_date', $currentDate->format('Y-m-d'))
    //                             ->first();
    ...
    //                             $row['first_row_tel_temperature'] = $maxTempEntry->min_temp;
    //                         } else {
    //                             $row['first_row_tel_temperature'] = 'Not Found';
    //                         }
    //                     }

    //                     // if($row['first_row_tel_temperature']=='N/A' && is_numeric($row['second_row_tel_temperature'])){
    //                     //     $row['first_row_tel_temperature'] = 'Bbbb';
    //                     // }
    //                     $i++;
    //                 }
    //             } else {
    //                 $row['first_row_date'] = $currentDate->format('Y-m-d');
    //                 if($row['first_row_tel_temperature']== 'N/A' && $row['second_row_tel_temperature']=='N/A'){
    //                     $row['first_row_tel_temperature'] = 'NW';
    //                     $row['second_row_tel_temperature'] = 'NW';
    //                 }
    //             }
    ...
    //                         $row['second_row_time'] = $chamberEntry->time;
    //                         $row['second_row_gps_time'] = $chamberEntry->gps_time;
    //                         $row['second_row_tel_temperature'] = $chamberEntry->tel_temperature;
    //                     }
    //                     $i++;
    //                 }
    //             } else {
    ...
    private function processChambersData_bck_ori($unique_id, $startDate, $endDate)
    {
        $chambers = Chamber::whereIn('sys_service_id', $unique_id)
            ->whereBetween('date', [
                $startDate->format('Y-m-d'),
            ...
            while ($currentDate->lte($endDate)) {
                ...
                if ($entries->isNotEmpty()) {
                    ...
                    if (is_numeric($row['first_row_tel_temperature']) && $row['second_row_tel_temperature'] == 'N/A') {
                        ...
                        if ($maxTempEntry) {
                            $row['second_row_tel_temperature'] = $maxTempEntry->max_temp . " NW_Afternoon";
                            ...
                            if ($telemetryData) {
                                ...
                                for ($i = 0; $i < 720; $i++) {
                                    ...
                                    // Insert into telemetry_new_chamber for this specific time
                                    DB::table('telemetry_new_chamber')->insert([
                                        'sys_service_id' => $telemetryData->sys_service_id,
                                        'sys_msg_type' => 1,
                                        'sys_proc_time' => $currentDate->format('Y-m-d') . ' ' . $currentGpsTime->format('H:i:s'),
                                        ...
                                        'tel_input_2' => $telemetryData->tel_input_2 ?? null,
                                        'tel_input_3' => $telemetryData->tel_input_3 ?? null,
                                        //'tel_temperature' => $telemetryData->tel_temperature ?? 0.0,
                                        'tel_temperature' => $maxTempEntry->max_temp,
                                        'tel_voltage' => $telemetryData->tel_voltage ?? 0.0,
                                        'main_powervoltage' => $telemetryData->main_powervoltage ?? 0.0,
                                        'tel_odometer' => $telemetryData->tel_odometer ?? null,
                    ...
                    if ($row['first_row_tel_temperature'] == 'N/A' && is_numeric($row['second_row_tel_temperature'])) {
                        ...
                        if ($maxTempEntry) {
                            $row['first_row_tel_temperature'] = $maxTempEntry->min_temp . " NW_Morning";
                        } else {
                            $row['first_row_tel_temperature'] = 'Not Found';
                        }
                    }
    ...
    private function processChambersData_bck2($unique_id, $startDate, $endDate)
    {
        $chambers = Chamber::whereIn('sys_service_id', $unique_id)
            ->whereBetween('date', [
                $startDate->format('Y-m-d'),
        ...
        foreach ($unique_id as $service_id) {
            ...
            while ($currentDate->lte($endDate)) {
                ...
                if ($entries->isNotEmpty()) {
                    ...
                    if (is_numeric($row['first_row_tel_temperature']) && $row['second_row_tel_temperature'] == 'N/A') {
                        $maxTempEntry = DB::table('latest_telemetry')
                            ->leftJoin('temperature_summary', 'temperature_summary.sys_service_id', '=', 'latest_telemetry.sys_service_id')
                            ->select('temperature_summary.max_temp')
                            ->where('temperature_summary.sys_service_id', $service_id)
                            ->where('temperature_summary.temp_date', $currentDate->format('Y-m-d'))
                            ->first();
                        ...
                        if ($maxTempEntry) {
                            $row['second_row_tel_temperature'] = $maxTempEntry->max_temp . " NW_Afternoon";
                            $startTime = Carbon::createFromFormat('Y-m-d H:i', '2024-02-03 00:00');

                            for ($i = 0; $i < 1440; $i++) {
                                $currentTime = $startTime->copy()->addMinutes($i);
                                // $row['temperature_times'][] = $currentTime->format('Y-m-d H:i');
    ...
    private function processChambersData_bck1($unique_id, $startDate, $endDate)
    {
        $chambers = Chamber::whereIn('sys_service_id', $unique_id)
            ->whereBetween('date', [
                $startDate->format('Y-m-d'),
            ...
            while ($currentDate->lte($endDate)) {
                ...
                if ($entries->isNotEmpty()) {
                    ...
                    if (is_numeric($row['first_row_tel_temperature']) && $row['second_row_tel_temperature'] == 'N/A') {
                        ...
                        if ($maxTempEntry) {
                            $row['second_row_tel_temperature'] = $maxTempEntry->max_temp . "Amar";
